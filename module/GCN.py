...
class GCNLayer(nn.Module):
	...
	def __init__(self,  channel_in, Ks, channel_out):
		...
		self.W = nn.Parameter(tc.zeros(size=(self.Ks*channel_in,channel_out)))
	...
	def forward(x,kernel):
		...
		x = x.contiguous().view(-1,n,channel_in).permute(0,2,1).view(-1,n)
	
		x_kernel = tc.mm(x,kernel).view(-1,channel_in,self.Ks,n).permute(0,3,1,2).contiguous().view(-1,channel_in*self.Ks)
		
		x_gconv = F.relu(tc.mm(x_kernel,self.W))          #F(H_next)  = acti(kernel*x*W)                                    
...
class GCN(nn.Module): #  GCN with single layer
	...
	def __init__(self, channel_in, Ks, channel_out):
		...
		self.W_first_order      = nn.Parameter(tc.zeros(size=(self.channel_in,channel_out)))
